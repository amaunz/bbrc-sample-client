...
begin
  for i in start_seed..end_seed
    begin 
      ...
      e4Sum = 1.0-commonFractionKV(fd_effects, md_effects)
      e5Sum = 1.0-correctSignFraction(fd_pValues, md_pValues, 0.95)

      puts "[#{Time.now.iso8601(4).to_s}] pValue difference (E1): #{sum_E1}"
      puts "[#{Time.now.iso8601(4).to_s}] pValue difference (E2): #{sum_E2}"
      puts "[#{Time.now.iso8601(4).to_s}] effects error (E4): #{e4Sum}"
      puts "[#{Time.now.iso8601(4).to_s}] significance error (E5): #{e5Sum}"
      $stdout.flush
